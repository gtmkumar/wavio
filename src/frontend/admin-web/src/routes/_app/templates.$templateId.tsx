...
function TemplateDetailPage() {
  ...
  async function onDelete() {
    ...
    try {
      ...
      await guard(() => remove.mutateAsync(templateId));
  ...
  return (
    <Sheet
      ...
      description={
        t ? `${t.category} · ${t.language}${t.metaTemplateId ? ` · Meta ID ${t.metaTemplateId}` : ""}` : undefined
      }
      actions={
        t ? (
          <>
            ...
            {hasPermission("templates.submit") && (t.status === "DRAFT" || t.status === "REJECTED") ? (
              <Button size="sm" onClick={() => void onSubmitToMeta()} disabled={submit.isPending}>
    ...
    >
      {template.isPending ? (
        ...
        <ErrorState error={template.error} />
      ) : (
      ...
      <Dialog
        ...
        description={`"${t?.name ?? "This template"}" will be removed. Campaigns already referencing its versions keep their history.`}
      >
