...
function CampaignDetailPage() {
  ...
  if (campaign.isPending) return <LoadingRows count={6} />;
  if (campaign.isError) return <ErrorState error={campaign.error} />;
  ...
  async function run(action: "launch" | "cancel") {
    ...
    try {
      ...
      await guard(() => (action === "launch" ? launch : cancel).mutateAsync(c.id));
  ...
  return (
    <>
      <Link to="/campaigns" className="mb-4 inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" /> All campaigns
      </Link>
      <PageHeader
        title={c.name}
        description={`Created for ${formatNumber(c.audienceCount)} recipients`}
        actions={
          <>
            <StatusBadge status={c.status} />
            {hasPermission("campaigns.launch") && (status === "draft" || status === "scheduled") ? (
              <Button onClick={() => setConfirm("launch")} disabled={launch.isPending}>
                <Rocket /> Launch
              </Button>
            ) : null}
            {hasPermission("campaigns.cancel") && ACTIVE_STATUSES.has(status) ? (
              <Button variant="destructive" onClick={() => setConfirm("cancel")} disabled={cancel.isPending}>
                <Ban /> Cancel
              </Button>
            ) : null}
          </>
      ...
      <Dialog
        ...
        description={
          confirm === "launch"
            ? `This starts sending to ${formatNumber(c.audienceCount)} recipients (minus suppressions) immediately.`
