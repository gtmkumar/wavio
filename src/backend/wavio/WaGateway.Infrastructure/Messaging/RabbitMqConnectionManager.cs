...
/// WaIntel's <c>RabbitMqConnectionManager</c> — duplicated rather than shared, matching this
/// project's existing convention of each bounded context owning its own thin messaging layer.
/// </summary>
public sealed partial class RabbitMqConnectionManager : IAsyncDisposable
{
    ...
    /// <summary>
    /// Fails closed outside Development when unconfigured — never silently falls back to a
    /// would-be-default local broker in production. This is the WaIngest S2 security-review
    /// lesson (issue #13): the earlier connection managers fell back to
    /// <see cref="DevelopmentOnlyFallback"/> unconditionally, which would have silently pointed a
    /// production deployment at a non-existent local broker instead of failing loudly at boot.
    /// </summary>
