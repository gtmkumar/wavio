// ─────────────────────────────────────────────────────────────────────────────
// WaIntel.WebApi (wa-intel-svc)
// Quality Rating Guardian, session window state, analytics event store, AI orchestration gateway (Waves 1-4)
//
// Listening port: http://localhost:5105 (dev; see launchSettings.json)
//
// Composition root: wires the wa-intel-svc bounded-context layers
//   • AddWaIntelApplication()    → use cases / handlers / validators
//   • AddWaIntelInfrastructure() → persistence, Meta Graph clients, bus publishers
...
using WaIntel.Application;
using WaIntel.Infrastructure;
...
builder.Services
    ...
    .AddWaIntelInfrastructure(builder.Configuration); // data surface + external clients
...
// Root liveness.
app.MapGet("/", () => "WaIntel.WebApi (wa-intel-svc) — up");
