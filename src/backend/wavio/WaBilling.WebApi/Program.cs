// ─────────────────────────────────────────────────────────────────────────────
// WaBilling.WebApi (wa-billing-svc)
// PMP cost ledger, tenant metering, quotas, invoicing feed, max-price bid config (Wave 2, #19)
//
// Listening port: http://localhost:5104 (dev; see launchSettings.json)
//
// Composition root: wires the wa-billing-svc bounded-context layers
//   • AddWaBillingApplication()    → use cases / handlers / validators
//   • AddWaBillingInfrastructure() → persistence, Meta Graph clients, bus publishers
...
using WaBilling.Application;
using WaBilling.Infrastructure;
...
// ── RabbitMq config (same guard as WaIntel/WaIngest Program.cs): fail closed outside Development ──
// RabbitMqConnectionManager (WaBilling.Infrastructure) independently refuses the same
// guest:guest@localhost fallback outside Development — this is the fast, eager, boot-time half of
// that same guard, so a missing broker config is caught before the host ever accepts traffic
// rather than silently talking to a would-be-default local broker in production.
...
// ── wa-billing-svc bounded-context composition ───────────────────────────────────────
builder.Services
    .AddWaBillingApplication()                          // validators + command/query handlers (no mediator)
    .AddWaBillingInfrastructure(builder.Configuration); // data surface + external clients
...
// Root liveness.
app.MapGet("/", () => "WaBilling.WebApi (wa-billing-svc) — up");
