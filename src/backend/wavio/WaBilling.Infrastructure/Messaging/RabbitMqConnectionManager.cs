...
/// reconnecting on demand when the cached connection has dropped. Same pattern as WaIntel's /
/// WaIngest's connection manager of the same name (issue #19) — each service owns its own copy
/// rather than sharing a cross-service project reference.
/// </summary>
public sealed partial class RabbitMqConnectionManager : IAsyncDisposable
{
    ...
    /// would-be-default local broker in production (same guard as WaIntel/WaIngest's connection
    /// managers). WaBilling.WebApi's Program.cs performs the same check eagerly at boot; this is
    /// the second, independent layer.</summary>
